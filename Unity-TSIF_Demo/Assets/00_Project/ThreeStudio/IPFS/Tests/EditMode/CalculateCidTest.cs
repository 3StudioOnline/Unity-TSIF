...
namespace ThreeStudio.IPFS.Tests
{
    ...
    public class CalculateCidTest
    {
        ...
        {
            byte[] data = await File.ReadAllBytesAsync(TestUtils.GetTestResourceFilepath("3S Studio.png"));
            (bool success, string errorMessage, string cid) result = await IpfsFunctionLibrary.CalculateCidFromDataForWeb3(data);
        ...
        {
            string filepath = TestUtils.GetTestResourceFilepath("3S Hello.txt");
            (bool success, string errorMessage, string cid) result = await IpfsFunctionLibrary.CalculateCidFromFileForWeb3(filepath);
