...
namespace ThreeStudio.IPFS
{
    public static partial class IpfsFunctionLibrary
    {
        ...
        {
            (bool success, string errorMessage, HttpResponse response, byte[] data) result = await GetData(ipfsHttpGatewayConfig, ipfsAddress);
        ...
        public static async Task<(bool success, string errorMessage, HttpResponse response, string dataString)> GetDataAsString(
            IpfsHttpGatewayConfig ipfsHttpGatewayConfig,
            IpfsAddress ipfsAddress)
        {
            (bool success, string errorMessage, HttpResponse response, byte[] data) result = await GetData(
